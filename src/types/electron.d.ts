...
declare global {
  interface Window {
    electronAPI?: {
      ...
      saveFile: (data: { filename: string; content: string; outputDir: string }) => Promise<{ success: boolean; error?: string }>;
      saveBinaryFile: (data: { filename: string; buffer: number[]; outputDir: string }) => Promise<{ success: boolean; error?: string }>;
