...
def request_chat_completion(
    *,
    ...
    payload_dict = {
        "model": model,
        "temperature": max(0.01, temperature), # Qwen models often prefer > 0
        "messages": messages,
        "max_tokens": 2000,
    }
    ...
    try:
        ...
        raise ValueError(f"HTTP {err.code}: {body}") from err
    except Exception as err:
        raise ValueError(f"Request failed: {err}") from err
...
def verify_model_endpoint(*, base_url: str, api_key: str, model: str, timeout_sec: int) -> None:
    ...
    _ = request_chat_completion(
        ...
        model=model,
        messages=[
            {"role": "user", "content": "You are a professional software localization translator. Translate to English: 你好"},
        ],
        temperature=0.0,
    ...
    if output_path.exists() and not force:
        ...
        for key, source_value in source_entries:
            ...
            if (
                ...
                and (allow_cjk or not CJK_RE.search(existing_value))
            ):
